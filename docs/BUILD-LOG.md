...
Components 26 → 29. Rule sets 6 → 9 (MD-001..006, INF-001..007, OKF-001..005).

#### Two more, found by opening the files rather than by running the suite

Both shipped to production, both were caught by reading the generated bytes.

- **Zero of 419 llms.txt links were rewritten.** `rewrite_links` compared the
  link's host against the site's verbatim. The site is filed as `redspot.com.au`
  and every page it lists is `www.redspot.com.au`, so every link was treated as
  off-site. The file came back **byte-for-byte identical**, which is also what
  "nothing needed changing" looks like — and `md/` scored **100/100** the whole
  time, because nothing checked the one thing the directory exists for. Now keyed
  on `text.domain_of` (the rule `58b4107` applied to the tables three days
  earlier), and **MD-006** counts links that resolve to a generated twin.
- **A literal `>` in the client's meta description.** The stored summary is
  pre-quoted on this run; `render._unquoted` was written for exactly that and was
  private to the llms.txt renderer. So llms.txt shipped clean and the two new
  artifacts shipped `description: "> Australian car rental company..."` and
  `<meta name="description" content="&gt; Australian...">`. Moved to
  `text.unquoted`; `render._unquoted` delegates rather than keeping a second copy.

**Redspot, verified on disk after staging:**

| | |
|---|---|
| llms.txt links | 419, all pointing at `.md` |
| resolving to a file in `md/` | **419 of 419** |
| `md/` | 419 files, 5.0 MB |
| `okf/` | 426 files, 1.1 MB, **0** broken internal links |
| `ai-info.html` | 2.5 KB, INF 100/100 |

**Measured:** 1,378 tests, 1 xfailed, ruff check and format clean.
Deployed through `a657d4f8` (web) / `3626fdbb` (worker), 14:11 AEST.
